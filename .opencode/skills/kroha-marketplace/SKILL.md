...
- Запустить сервис (Windows PowerShell):
  ```powershell
  Start-Process -FilePath "php" -ArgumentList "-S","127.0.0.1:8091" -WorkingDirectory "<корень проекта>" -WindowStyle Hidden
  ```
...
4. `php tests/run_tests.php` — набрать 97/97 OK.
...
- `tests/run_tests.php` — автономный раннер 97 проверок на `sqlite::memory:`; свои хелперы: `check()`, `assert_eq()`, `fresh_db()`, `section()`.
...
- После изменения **любой** страницы: `php -l <файл>` и `php tests/run_tests.php` (97/97).
...
- Тесты: `php tests/run_tests.php`; норма — все `OK`, счётчики `tests=97 failed=0 skipped=0`.
...
- При изменении количества тестов — поменять «97 проверок» и в README, и в `tests/run_tests.php`, и этой цифрой не разбрасываться.
